...
contract("ERC20", function (accounts) {
  ...
  describe("mint", function () {
    ...
    it("rejects a null account", async function () {
      await expectRevert(
        this.token.mint(ZERO_ADDRESS, amount, { from: initialHolder }),
    ...
    describe("for mint finished", function () {
      beforeEach("finishMinting", async function () {
        const { logs } = await this.token.finishMinting({
          from: initialHolder,
        });
        this.logs = logs;
      });

      it("rejects mint tokens", async function () {
        await expectRevert(
          this.token.mint(recipient, amount, {
            from: initialHolder,
          }),
          "ERC20: Minting is finished"
        );
      });

      it("emits MintFinished event", async function () {
        const event = expectEvent.inLogs(this.logs, "MintFinished", {});
      });
    });
  });
